...
class FishWorldFun extends World {
  ...
  FishWorldFun(Player player, ILoFish bgfish, Random rand) {
    super();
    this.player = player;
    this.bgfish = bgfish;
    this.rand = rand;
  ...
  // constructor for real games
  FishWorldFun(Player player, ILoFish bgfish) {
    this(player, bgfish, new Random());
  }
...
interface ILoFish {
  ...
  // determines whether or not to add a new fish to the world and moves every
  // bgfish in the list
  ILoFish doOnTick(int generator);
...
class MtLoFish implements ILoFish {
  ...
  public ILoFish doOnTick(int generator) {
    if (generator % 5 == 0) {
      return new ConsLoFish(new BGFish(generator), this.moveAllFish());
    }
    else {
...
abstract class AFish implements IFish {
  ...
  public boolean comparePositions(AFish that) {
    return Math.sqrt(Math.pow(this.x - that.x, 2) + Math.pow(this.y - that.y, 2)) <= this.size
        + that.size;
  }
...
class BGFish extends AFish {
  ...
  BGFish(int generator) {
    ...
    int calcPoints = u.getPoints(generator % 11);
    this.x = u.placeX(right);
    this.y = generator % BACKGROUND_HEIGHT;
    this.size = u.getSize(calcPoints);
...
class ExamplesFish {
  ...
  ILoFish bgList5NotEatenp2 = new ConsLoFish(this.bg3Off, new ConsLoFish(this.bg2On,
      new ConsLoFish(this.bg3Edge, new ConsLoFish(this.bg2Edge, this.mt))));

  ILoFish bgList5NotEatenp3 = new ConsLoFish(this.bg3Off, new ConsLoFish(this.bg3Edge, this.mt));
  ...
  ILoFish bgList2BeforeMoved = new ConsLoFish(new BGFish(100, 150, 100, false),
      this.bgList1BeforeMoved);
  ILoFish bgList2Moved = new ConsLoFish(new BGFish(95, 150, 100, false), this.bgList1Moved);
  ILoFish bgList3BeforeMoved = new ConsLoFish(new BGFish(370, 150, 150, true),
      this.bgList2BeforeMoved);
  ILoFish bgList3Moved = new ConsLoFish(new BGFish(375, 150, 150, true), this.bgList2Moved);
  ILoFish bgList4BeforeMoved = new ConsLoFish(new BGFish(-30, 150, 150, false),
      this.bgList3BeforeMoved);
  ILoFish bgList5BeforeMoved = new ConsLoFish(new BGFish(100, 150, 150, true),
      this.bgList4BeforeMoved);
  ILoFish bgList5Moved = new ConsLoFish(new BGFish(105, 150, 150, true), this.bgList3Moved);

  // Random example
  Random randEx = new Random(1); // 185, 188, 47, 113, 54

  // World examples
  FishWorldFun emptyWorld = new FishWorldFun(p1, mt, randEx);
  FishWorldFun world1 = new FishWorldFun(p2, bgList1, randEx);
  ...
  WorldImage ocean = new RectangleImage(400, 200, OutlineMode.SOLID, Color.BLUE);
  WorldScene bg = this.emptyWorld.getEmptyScene().placeImageXY(ocean, 200, 100);
  ...
  boolean testOnKeyEvent(Tester t) {
    return t.checkExpect(this.emptyWorld.onKeyEvent("x"), this.emptyWorld.endOfWorld("Goodbye"))
        && t.checkExpect(this.world1.onKeyEvent("up"),
            new FishWorldFun(new Player(100, 95, 100), this.bgList1))
        && t.checkExpect(this.world1.onKeyEvent("left"),
            new FishWorldFun(new Player(95, 100, 100), this.bgList1))
        && t.checkExpect(this.world1.onKeyEvent("down"),
            new FishWorldFun(new Player(100, 105, 105), this.mt))
        && t.checkExpect(this.world1.onKeyEvent("right"),
            new FishWorldFun(new Player(105, 100, 100), this.bgList1))
        && t.checkExpect(this.world1.onKeyEvent("d"),
            new FishWorldFun(new Player(100, 100, 105), this.mt))
        && t.checkExpect(this.worldEats.onKeyEvent("right"), this.worldEatsAfter);
  }

  // test onTick method
  boolean testOnTick(Tester t) {
    ILoFish newBGFishTest = this.mt.doOnTick(185);
    ILoFish newBGFishTest2 = this.bgList1.doOnTick(188);
    return t.checkExpect(this.emptyWorld.onTick(),
        new FishWorldFun(newBGFishTest.filterByPlayer(new ByEaten(), this.p1).growByAll(this.p1),
            newBGFishTest.filterByPlayer(new ByNotEaten(), this.p1)))
        && t.checkExpect(this.world1.onTick(),
            new FishWorldFun(
                newBGFishTest2.filterByPlayer(new ByEaten(), this.p2).growByAll(this.p2),
                newBGFishTest2.filterByPlayer(new ByNotEaten(), this.p2)));
  }

  // test makeScene method
  boolean testMakeScene(Tester t) {
    return t.checkExpect(this.emptyWorld.makeScene(),
        this.mt.placeFish(this.bg).placeImageXY(this.p1.drawFish(), 100, 100))
        && t.checkExpect(this.worldEaten.makeScene(),
            this.bgList5.placeFish(this.bg).placeImageXY(this.p1.drawFish(), 100, 100));
  }

  // test lastScene method
  boolean testLastScene(Tester t) {
    return t.checkExpect(this.emptyWorld.lastScene("still swimming"),
        this.emptyWorld.makeScene().placeImageXY(new TextImage("still swimming", Color.BLACK), 100,
            40))
        ...
        && t.checkExpect(this.worldEaten.lastScene("You win! You're king of the fish."),
            this.worldWin.makeScene()
                .placeImageXY(new TextImage("You've been eaten! You lose!", Color.BLACK), 100, 40));
  }

  // test worldEnds method
  boolean testWorldEnds(Tester t) {
    return t.checkExpect(this.emptyWorld.worldEnds(),
        new WorldEnd(false, this.emptyWorld.makeScene()))
  ...
  boolean testFilterByPlayer(Tester t) {
    return t.checkExpect(this.mt.filterByPlayer(this.byEaten, this.p2), this.mt)
        ...
        && t.checkExpect(this.bgList5.filterByPlayer(this.byNotEaten, this.p2),
            this.bgList5NotEatenp2)
        && t.checkExpect(this.bgList5.filterByPlayer(this.byEaten, this.p2), this.bgList1)
        && t.checkExpect(this.bgList5.filterByPlayer(this.byNotEaten, this.p3),
            this.bgList5NotEatenp3)
  ...
  boolean testGrowByAll(Tester t) {
    return t.checkExpect(this.mt.growByAll(this.p1), this.p1)
        && t.checkExpect(this.bgList2.growByAll(this.p2), new Player(100, 100, 125))
        && t.checkExpect(this.bgList5.growByAll(this.p2), new Player(100, 100, 205))
        && t.checkExpect(this.bgList3.growByAll(p7), new Player(100, 100, 405));
  }

  // test doOnTick method
  boolean testDoOnTick(Tester t) {
    return t.checkExpect(this.mt.doOnTick(185),
        new ConsLoFish(new BGFish(u.BACKGROUND_WIDTH, 185, 225, false), this.mt))
        && t.checkExpect(this.bgList1.doOnTick(188), this.bgList1.moveAllFish())
        && t.checkExpect(this.mt.doOnTick(3), this.mt)
        && t.checkExpect(this.bgList1.doOnTick(185), new ConsLoFish(
            new BGFish(u.BACKGROUND_WIDTH, 185, 225, false), this.bgList1.moveAllFish()));
  }

  // test playerEaten method
  boolean testPlayerEaten(Tester t) {
    return t.checkExpect(this.mt.playerEaten(p3), false)
        && t.checkExpect(this.bgList2.playerEaten(p2), true)
        && t.checkExpect(this.bgList1.playerEaten(p2), false)
        && t.checkExpect(this.bgList1.playerEaten(p2), false)
        && t.checkExpect(this.bgList2.playerEaten(p2), true)
        && t.checkExpect(this.bgList5.playerEaten(p1), true);
  }
  ...
  boolean testPlaceFish(Tester t) {
    return t.checkExpect(this.mt.placeFish(bg), bg)
        && t.checkExpect(this.bgList1.placeFish(bg),
            bg.placeImageXY(this.bg1Edge.drawFish(), 90, 110))
        && t.checkExpect(this.bgList1.placeFish(bg),
            bg.placeImageXY(this.bg1Edge.drawFish(), 90, 110).placeImageXY(this.bg2Edge.drawFish(),
                120, 80));
  }
  ...
  boolean testDrawFish(Tester t) {
    return t.checkExpect(this.p2.drawFish(), new CircleImage(20, OutlineMode.SOLID, Color.GREEN))
        ...
        && t.checkExpect(this.bg1Edge.drawFish(),
  ...
  boolean testGetNewPosition(Tester t) {
    return t.checkExpect(this.p2.getNewPosition(20, "up"), 20)
        ...
        && t.checkExpect(this.p2.getNewPosition(u.BACKGROUND_HEIGHT + 20, "down"),
            u.BACKGROUND_HEIGHT + 20)
        ...
        && t.checkExpect(this.p2.getNewPosition(u.BACKGROUND_WIDTH + 20, "right"),
            u.BACKGROUND_WIDTH + 20)
  ...
  boolean testGetSize(Tester t) {
    return t.checkExpect(this.u.getSize(0), 10) && t.checkExpect(this.u.getSize(50), 10)
        && t.checkExpect(this.u.getSize(100), 20) && t.checkExpect(this.u.getSize(150), 30)
        && t.checkExpect(this.u.getSize(200), 40) && t.checkExpect(this.u.getSize(250), 50)
        && t.checkExpect(this.u.getSize(300), 60) && t.checkExpect(this.u.getSize(350), 70)
  ...
  boolean testPlaceX(Tester t) {
    return t.checkExpect(this.u.placeX(true), 0) && t.checkExpect(this.u.placeX(false), 400);
  }

  // test the getPoints method
  boolean testGetPoints(Tester t) {
    return t.checkExpect(this.u.getPoints(0), 25) && t.checkExpect(this.u.getPoints(1), 25)
        && t.checkExpect(this.u.getPoints(2), 25) && t.checkExpect(this.u.getPoints(3), 75)
        && t.checkExpect(this.u.getPoints(4), 75) && t.checkExpect(this.u.getPoints(5), 75)
        && t.checkExpect(this.u.getPoints(6), 125) && t.checkExpect(this.u.getPoints(7), 125)
        && t.checkExpect(this.u.getPoints(8), 175) && t.checkExpect(this.u.getPoints(9), 225)
        && t.checkExpect(this.u.getPoints(10), 275) && t.checkExpect(this.u.getPoints(20), 275);
  }
}
